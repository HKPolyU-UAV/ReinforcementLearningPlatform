...
class SecondOrderIntegration(rl_base):
    def __init__(self,
                 ...
                 is_controller_BangBang: bool = True):
        ...
        if self.is_controller_BangBang:
            # 为方便DQN学习，bangbang控制的动作将被以0开始进行编号，构成一维动作
            self.action_dim = 1
            self.action_num = [2 * 2]
            self.action_dict = {
                '0': np.array([self.fMin, self.fMin]), '1': np.array([self.fMin, self.fMax]),
                '2': np.array([self.fMax, self.fMin]), '3': np.array([self.fMax, self.fMax])
            }
            self.action_space = [[i for i in range(self.action_num[0])]]
    ...
    def get_reward(self, param=None):
        if self.is_controller_BangBang:
            ...
            if self.terminal_flag == 1:  # position out
                ...
                u_extra = _n * (u_pos + u_vel)

            self.reward = u_pos + u_vel + u_extra
    ...
    def step_update(self, action: np.ndarray):
        ...
        if self.is_controller_BangBang:
            action = self.action_dict[str(action[0])]
