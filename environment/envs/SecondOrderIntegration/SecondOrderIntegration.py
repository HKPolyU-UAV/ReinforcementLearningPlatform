...
class SecondOrderIntegration(rl_base):
	...
	def draw_ball(self):
		...
		if self.force[0] > 0:
			cv.circle(self.image, self.dis2pixel(self.pos - np.array([0.25, 0])), self.length2pixel(0.1), Color().Blue, -1)
			cv.line(self.image,
		...
		if self.force[1] > 0:
			cv.circle(self.image, self.dis2pixel(self.pos - np.array([0., 0.25])), self.length2pixel(0.1), Color().Blue, -1)
			cv.line(self.image,
