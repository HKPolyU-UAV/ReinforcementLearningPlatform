...
if __name__ == '__main__':
    ...
    env = env()
    eval_net = Actor(state_dim=env.state_dim, action_dim=env.action_dim, a_min=env.action_range[:, 0],
                     a_max=env.action_range[:, 1], alpha=1e-4)
    eval_net.load_state_dict(torch.load(optPath + 'target_actor'))
    video = cv.VideoWriter('../DDPG-4-' + env.name + '.mp4', cv.VideoWriter_fourcc(*"mp4v"), 200, (env.image_size[0], env.image_size[1]))
    n = 2
    for _ in range(n):
