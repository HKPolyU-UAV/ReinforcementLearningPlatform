	...
	while agent.memory.mem_counter < fullFillCount:
		...
		while not env.is_terminal:
			...
			if is_only_success:
				...
				_new_reward.append(r_norm(env.reward))
				_new_state_.append(env.next_state)
				_new_done.append(1 if env.is_terminal else 0)
			else:
				agent.memory.store_transition(env.current_state, env.current_action, r_norm(env.reward), env.next_state, 1 if env.is_terminal else 0)
