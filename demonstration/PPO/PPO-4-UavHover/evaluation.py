...
if __name__ == '__main__':
    ...
    '''1. 外环控制器加载'''
    policy_pos = PPOActorCritic(int(env.state_dim/2), int(env.action_dim/2), env.action_range[:3, :])
    policy_pos.load_state_dict(torch.load(optPath + 'pos-actor-critic'))
    '''2. 内环控制器加载'''
    policy_att = PPOActorCritic(int(env.state_dim/2), int(env.action_dim/2), env.action_range[3:, :])
    ...
    for _ in range(test_num):
        ...
        while not env.is_terminal:
            ...
            u_from_actor = policy_pos.evaluate(env.current_state[:6]).numpy()
            uf = policy_pos.action_linear_trans(u_from_actor.flatten())
            '''3.2 内环网络根据内环状态给出转矩'''
            torque_from_actor = policy_att.evaluate(env.current_state[6:]).numpy()
            torque = policy_att.action_linear_trans(torque_from_actor.flatten())
